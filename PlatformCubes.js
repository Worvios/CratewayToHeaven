...
function newPlatformPosition() {
  ...
  var y;
  if (x > 160) {
    y = Math.random()* 50 + 50 + platPos.y;
  } else if (x> 120){
    y = Math.random()* 35 + 80 + platPos.y;
  } else if (x > 75) {
    y = Math.random()* 50 + 80 + platPos.y;
  } else {
    y = Math.random()* 35 + 100 + platPos.y; 
  }
...
function makePlatform() {
    ...
    var material = new THREE.MeshPhongMaterial( { map: THREE.ImageUtils.loadTexture('images/crate2.jpg') } );
